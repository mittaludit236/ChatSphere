...
const ChatTop = ({ user, selected }) => {
    ...
    const handleAddSelectedUsers = () => {
        axios.post('/conversations/addM', { members: selectedUsers.map(user => user._id), conversationId: selected._id })
            .then((response) => {
    ...
    const handleRemoveSelectedUsers = () => {
        axios.post('/conversations/removeM', { members: selectedUsers.map(user => user._id), conversationId: selected._id })
            .then((response) => {
    ...
    const handleRemoveAdmin = (adminId) => {
        axios.post('/conversations/removeAd', { curId: user._id, memberId: adminId, conversationId: selected._id })
            .then((response) => {
    ...
    const handleAddAdmin = (adminId) => {
        axios.post('/conversations/makeAd', { curId: user._id, memberId: adminId, conversationId: selected._id })
            .then((response) => {
        ...
        <div className="flex justify-between items-center bg-gray-200 px-8 py-6 rounded-t-lg">
            <div className="flex items-center">
                {senderUsername && senderUsername.profilePicture && (
                    <img src={senderUsername.profilePicture} alt="Profile" className="rounded-full h-8 w-8 mr-2" />
                )}
                <h2 className="text-lg font-semibold text-black">
                    {isGroupChat ? selected?.chatName : senderUsername?.username || selected?.title || 'Chat'}
                </h2>
            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-lg p-6">
                        ...
                        {showSearch && (
                            <div className="mb-4 overflow-y-auto max-h-40">
                                <input type="text" placeholder="Search users" className="border rounded-lg px-2 py-1 w-full" value={searchInput} onChange={handleChange} />
                                <ul>
                                    {searchResults.map(user => (
                                        <li key={user._id} onClick={() => setSelectedUsers([...selectedUsers, user])} className="search-result flex items-center cursor-pointer">
                                            <img src={user.profilePicture ? PF + user.profilePicture : PF + "person/noadmin.webp"} alt="Profile" className="rounded-full h-8 w-8 mr-2" />
                                            {user.username}
                                        </li>
                                    ))}
                                </ul>
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex flex-wrap">
                                {selectedUsers.map((member) => (
                                    <div key={member._id} className="flex items-center mr-2 mb-2 selected-member">
                                        <img src={member.profilePicture ? PF + member.profilePicture : PF + "person/noadmin.webp"} alt="Profile" className="rounded-full h-8 w-8 mr-2" />
                                        <span>{member.username}</span>
                                        <button className="ml-2 px-2 py-1 bg-red-500 text-white rounded-full text-xs" onClick={() => setSelectedUsers(selectedUsers.filter(u => u._id !== member._id))}>Remove</button>
                                    </div>
                                ))}
                            </div>
                        </div>
                                    ...
                                    {memberUsers.map(user => (
                                        <li key={user._id} className="flex items-center mb-2">
                                            ...
                                            {groupAdmin.includes(user._id) ? (
                                                <button className="ml-2 text-red-500" onClick={() => handleRemoveAdmin(user._id)}>Remove Admin</button>
                                            ) : (
                                                <button className="ml-2 text-green-500" onClick={() => handleAddAdmin(user._id)}>Add Admin</button>
                                            )}
                                        </li>
                                    ))}
                                </ul>
                        ...
                        <button className="bg-blue-500 text-white py-2 px-4 rounded-lg mr-2" onClick={showSearch ? handleAddSelectedUsers : handleAddUser}>
                            {showSearch ? 'Add Selected Users' : 'Add User'}
                        </button>
                        <button className="bg-red-500 text-white py-2 px-4 rounded-lg mr-2" onClick={showSearch ? handleRemoveSelectedUsers : handleRemoveUser}>
